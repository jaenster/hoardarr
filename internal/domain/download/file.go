...
// IsRecoveryVol reports whether this file is a PAR2 recovery-volume
// (filename matches `<base>.vol###+##.par2`). When a Job has
// `fetch_recovery_vols=false`, the orchestrator skips segments
// belonging to recovery-vol files until the repair worker explicitly
// requests them. The small index PAR2 (no `.vol`) stays
// IsRecoveryVol=false even though IsPar2() is true.
func (f *File) IsRecoveryVol() bool { return f.isRecoveryVol }
